...
- Keeps your **master schema** exactly the same.
- **NJT**: fills Scheduled_Arrival (and Arrival_Delay_Min) via trip_id-agnostic merge
...
- **MTA/Subway**: **preserves** any existing Scheduled_Arrival / Scheduled_Departure
  fields produced by your Subway pipeline. If they don’t exist in the RT CSVs,
  we leave them blank (no regression). That keeps the spreadsheet consistent
  across both sources without breaking your current Subway behavior.
- Safe merge_back (suffix + coalesce) and numeric `merge_asof` tolerance (no
  pandas MergeError). TZ-safe conversions (no `.view` warnings).
...
MTA_RT_GLOB        = "*.csv"          # keep broad; we preserve fields if present
...
# -------------------- RT ⇄ schedule time alignment (NJT) --------------------
...
def _sec_to_local_utc(local_date: pd.Series, sec_col: pd.Series) -> pd.Series:
    ...
    If seconds are NaN, result remains NaT (no midnight defaulting).
    """
    base = pd.to_datetime(local_date.astype(str), errors="coerce")
    base = base.dt.tz_localize(NY_TZ)
...
def join_njt_rt_to_schedule(rt: pd.DataFrame, sched: pd.DataFrame, tolerance_min: int = 45) -> pd.DataFrame:
    ...
    rt["local_date_dep"], rt["rt_dep_sec"] = _utc_to_local_seconds(rt["rt_departure_utc"])  # may be NaN
    ...
    # ---------------- Departure-based nearest match (to fill Scheduled_Departure) ----------------
    ...
    # Build Scheduled_Departure UTC from (local_date_dep, sched_dep_key_sec)
    ...
    # Attach departure schedule + optional trip_id fill
    ...
    # Prefer arrival trip_id, else fill from departure pass
    ...
    # Scheduled_Departure filled via departure-based join above
...
def build_njt_interfaces() -> pd.DataFrame:
    ...
    out = out.reindex(columns=MASTER_COLUMNS + ["trip_id","stop_id","route_id"])  # keep keys for join
...
# -------------------- Optional MTA static schedule loader (auto-detect) --------------------
def maybe_load_mta_static_schedule() -> pd.DataFrame:
    """Load MTA static schedule.
    Priority:
      1) MTA_STATIC_ZIP env path, if provided (optional)
      2) Auto-detect newest zip under gtfs_static/: mta_subway_static*, mta_subway*, mta_*, *.zip
    Returns empty DataFrame if nothing found or unreadable.
    """
    zpath = None
    ...
    if env:
        ...
        if p.exists():
            ...
            print(f"[mta][sched] static zip not found: {env}")
    if zpath is None:
        root = pathlib.Path("gtfs_static")
...
# -------------------- Generic nearest-time join (used for MTA static) --------------------
def join_generic_nearest(rt_arrival_utc: pd.Series, rt_departure_utc: pd.Series,
    ...
    sched_arrival = _sec_to_local_utc(mergedA["local_date_arr"], mergedA["sched_key_sec"]) if "sched_key_sec" in mergedA else pd.Series(pd.NaT, index=rt.index)
    # Departure
    ...
    sched_departure = _sec_to_local_utc(mergedD["local_date_dep"], mergedD["sched_dep_key_sec"]) if "sched_dep_key_sec" in mergedD else pd.Series(pd.NaT, index=rt.index)
...
def build_mta_interfaces() -> pd.DataFrame:
    ...
    # Always try static GTFS if available; fill only gaps
    ...
    if not mta_sched.empty:
        ...
        before_na_arr = sched_arr.isna().sum(); before_na_dep = sched_dep.isna().sum()
        sched_arr = sched_arr.combine_first(sa)
        sched_dep = sched_dep.combine_first(sd)
        after_na_arr = sched_arr.isna().sum(); after_na_dep = sched_dep.isna().sum()
        print(f"[mta][sched] static used; filled Scheduled_Arrival {before_na_arr - after_na_arr}, Scheduled_Departure {before_na_dep - after_na_dep}")
        used_static = True
    else:
        print("[mta][sched] no static schedule available; using only provided Scheduled_* fields")

    # Compute delays when both RT and Scheduled exist; keep NaN if either missing
    ...
    # Interface_ID — keep simple & deterministic for Subway
