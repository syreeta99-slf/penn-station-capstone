...
# -------------------- MTA/Subway realtime → interfaces (preserve schedule) --------------------
def build_mta_interfaces() -> pd.DataFrame:
    ...
    # We PRESERVE existing scheduled fields if present (no regression).
    # Expected / optional columns in RT exports:
    #  - rt_arrival_utc, rt_departure_utc
    #  - Scheduled_Arrival, Scheduled_Departure (already computed by your pipeline)
    #  - From_Node/To_Node (we will default if missing)

    # Normalize column presence
    for c in ["rt_arrival_utc","rt_departure_utc","Scheduled_Arrival","Scheduled_Departure"]:
        if c not in df.columns:
            df[c] = pd.NA

    # Build interface rows
    rt_arr = _to_dt_utc(df["rt_arrival_utc"]) if "rt_arrival_utc" in df else pd.NaT
    rt_dep = _to_dt_utc(df["rt_departure_utc"]) if "rt_departure_utc" in df else pd.NaT

    sched_arr = _to_dt_utc(df["Scheduled_Arrival"]) if "Scheduled_Arrival" in df else pd.NaT
    sched_dep = _to_dt_utc(df["Scheduled_Departure"]) if "Scheduled_Departure" in df else pd.NaT
    ...
    interface_id = (
        "MTA_Subway_" + SUBWAY_PENN_NODE + "_" + stamp_min.fillna("NA")
    )
    ...
    out = _ensure_columns(out, MASTER_COLUMNS)
    print(f"[mta] built {len(out)} MTA interface rows (preserved Scheduled_* if present)")
    return out
