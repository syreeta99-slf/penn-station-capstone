...
- Output:  Append normalized rows to CSV per UTC day, optionally filtered by stop_id(s)
...
Env vars (optional):
  ...
  NJT_ENV              (default "prod")
  NJT_FILTER_STOP_IDS  (comma-separated, default "105")  # strict Penn filter by default
...
# GTFS-RT fetch/parse
# -----------------------------
def _to_iso_utc(epoch) -> Optional[str]:
    if epoch is None:
        return None
    ts = float(epoch)
    if ts > 1e11:
    ...
    for ent in feed.entity:
        ...
        for stu in tu.stop_time_update:
            ...
            rows.append(
                {
                    "trip_id": trip_id,
                    "stop_id": str(stop_id) if stop_id is not None else "",
                    "route_id": route_id,
...
def _apply_stop_filter(rows: List[dict]) -> List[dict]:
...
# -----------------------------
def main() -> None:
    log.info(f"Filter stop_ids: {sorted(FILTER_STOP_IDS) if FILTER_STOP_IDS else 'NONE (all)'}")
    ...
    if status is TokenStatus.OK and token:
        try:
            ...
            filtered = _apply_stop_filter(rows)
            log.info(f"Parsed {len(rows)} rows; kept {len(filtered)} after stop filter")
            _append_daily_csv(filtered)
            sys.exit(0)
